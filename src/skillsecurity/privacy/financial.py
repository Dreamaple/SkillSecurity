...
_PAYMENT_PATTERNS: list[FinancialPattern] = [
    ...
    FinancialPattern(
        ...
        "PayPal Payment API",
        "payment_api",
        url_pattern=re.compile(r"api\.paypal\.com/(v1|v2)/(payments|orders|billing)"),
        method="POST",
    ),
    FinancialPattern(
        "alipay",
    ...
    FinancialPattern(
        ...
        "WeChat Pay API",
        "payment_api",
        url_pattern=re.compile(r"api\.mch\.weixin\.qq\.com/(v3/)?(pay|transactions)"),
        method="POST",
    ),
]
...
_CLOUD_PATTERNS: list[FinancialPattern] = [
    ...
    FinancialPattern(
        ...
        param_pattern=re.compile(r"Action=(Create|Launch|Purchase|Subscribe)"),
        severity="high",
    ),
    FinancialPattern(
        "gcp-compute-create",
    ...
    FinancialPattern(
        ...
        "Azure VM Creation",
        "cloud_resource",
        url_pattern=re.compile(r"management\.azure\.com/.*/virtualMachines"),
        method="PUT",
        severity="high",
    ),
]
...
class FinancialDetector:
    ...
    def __init__(self) -> None:
        self._patterns = _PAYMENT_PATTERNS + _CLOUD_PATTERNS + _CRYPTO_PATTERNS + _BROWSER_PATTERNS
